...
final class Mobile
{
        ...
        if (count($data) > 0) { //Duplicate username or mobile
            ...
            try {
                ...
                if ($gender == '1' || $gender == 1) {
                    $title = 'زفاف ترحب بك';
                    $message = 'لمزيد من التفاعل معك يرجى الإشتراك في الباقة الوردية';
                } else {
                    $title = 'زفاف ترحب بك';
                    ...
                    $message = 'لمزيد من التفاعل معك يرجى الإشتراك بالعضوية الماسية';
                }
                $params['title'] = $title;
    ...
    public function changeMyStatus(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        public function updateUserDetails(Request $request, Response $response, $args, $internal = 0)
    {
        $responseData = array();
        $inputs = $request->getParsedBody();
        $userId = $request->getAttribute("userId");
        ...
        // Building the SQL set part dynamically
        $setPart = [];
        $params = [':userId' => $userId];
        foreach ($fields as $field) {
            if (array_key_exists($field, $inputs) && $inputs[$field] != null) {
                $setPart[] = "$field = :$field";
                $params[":$field"] = $inputs[$field];
            }
        }


        //check if there is not data sent
        if (empty($setPart)) {
            $responseData['status'] = 'error';
            $responseData['errorCode'] = 'No valid fields to update';
        } else {
        
            $sql = "UPDATE users SET " . implode(", ", $setPart) . " WHERE id = :userId";

            try {
                $sth = $this->db->prepare($sql);
                $sth->execute($params);
                
                // Additional update to set canUpdateProfile=1
            $sql1 = "UPDATE users SET canUpdateProfile=1 WHERE id = :userId";
            ...
            $sth->execute([':userId' => $userId]);
                ...
                $sql = "SELECT cityId, mariageKind, mariageStatues, aboutMe,aboutOther, workField, job, income 
                    FROM users 
                    WHERE id = :userId";
                $sth = $this->db->prepare($sql);
                $sth->execute([':userId' => $userId]);
    ...
    public function availabilityToEditProfile(Request $request, Response $response, $args, $internal = 0)
    {
        $responseData = array();
        $inputs = array();
        $inputs = $request->getParsedBody();
        ...
        $sql = "select canUpdateProfile from users where id=$userId";
        $responseData = array();
        ...
        try {
            ...
            $data = $sth->fetchAll();
            $responseData['status'] = 'success';
            $responseData['rowsCount'] = $data[0]['canUpdateProfile'];
    ...
    public function getUserDetails(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $id = $args['id'];
        // Tamer 483 Mona 381
        $sql = " select users.id,packages.packageLevel,userName,CONCAT(users.mobileCode, users.mobile) as mobile,gender,if(allowImage=1,profileImage,'') as userImage,available,
    ...
    public function getPackagesV2(Request $request, Response $response, $args, $internal = 0)
    {
        $userId = $request->getAttribute("userId");
        ...
        try {
            ...
            $sql = " select telesalesId,residentCountryId, gender from users where users.id = $userId ";
            $sth = $this->db->prepare($sql);
            $sth->execute();
            $queryResult = $sth->fetchAll();
            $telesalesId = $queryResult[0]['telesalesId'];
            $countryId = $queryResult[0]['residentCountryId'];
            $gender = $queryResult[0]['gender'];
            //if gender == 1 (female) return packages for females and vice versa
            $packageType = ($gender == 1) ? 1 : 0;


            $sql = " select * from packages where active=1 and countryId=0 and packageType=$packageType";
            ...
            $sql = " select * from packages where active=1 and countryId=$countryId and packageType=$packageType ";
            $sql .= " order by usdValue desc ";
            $sth = $this->db->prepare($sql);
            $sth->execute();
    ...
    public function purchasePackage(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $sql1 = "update users set PreviousPackageRenewDate = packageRenewDate ,previousPackageId = packageId,
             ...
             INTERVAL (select validFor from packages where id=$packageId) DAY) ,packageId=$packageId, packageMobileRequestLimit = (select mobileRequestLimit from packages where id=$packageId), canUpdateProfile=0 where users.id=$userId";
    ...
    public function getMessagesList(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $sth->execute();
        $data = $sth->fetchAll();
        $residentCountryId = $data[0]['residentCountryId'] ?? null;

        $sql = "select messages.id,userId,messageDateTime,reasonId,title,message,userReaded as readed,JSON_UNQUOTE(multi_image) AS multi_image,
        ...
        try {
            ...
            // Decode multi_image JSON string into an array
            $data[$i]['multi_image'] = $data[$i]['multi_image'] != null ? json_decode($data[$i]['multi_image'], true): $data[$i]['multi_image'];
      ...
      public function getMessagesList2(Request $request, Response $response, $args, $internal = 0)
    {
        $userId = $request->getAttribute("userId");
        $userGender = $request->getAttribute("userGender");
        $sql = "select residentCountryId  from users where id=$userId";
       ...
       if ($reasonId >= 0) {
             $reasonIdCondition = "and messages.reasonId = $reasonId";
       } else {
            $reasonIdCondition = "and (messages.reasonId in (0, 1, 2, 3))";
       }
        $sql = "select messages.id,userId,messageDateTime,reasonId,title,message,userReaded as readed,JSON_UNQUOTE(multi_image) AS multi_image,
                owner,image,adminImage,otherId,if(reply='',NULL,reply) as reply 
                from messages inner join users on
                messages.userId = users.id
                 ...
                 (userId=$userId or userId=0) and 
                ( messages.residentCountryId=$residentCountryId or messages.residentCountryId=0) 
                and (messages.mesGender = $userGender or mesGender=2) $reasonIdCondition
    ...
    public function getMarriageRequestsList(Request $request, Response $response, $args, $internal = 0)
    {
        $userId = $request->getAttribute("userId");
        $userGender = $request->getAttribute("userGender");
        $sql = "select residentCountryId  from users where id=$userId";
        ...
        $sql = "select messages.id,userId,messageDateTime,reasonId,title,message,userReaded as readed,JSON_UNQUOTE(multi_image) AS multi_image,
                ...
                messages.userId = users.id
                 ...
                 (userId=$userId or userId=0) and 
                ( messages.residentCountryId=$residentCountryId or messages.residentCountryId=0) 
                and (messages.mesGender = $userGender or mesGender=2) and (messages.reasonId = 4)
    ...
    public function openChatV1(Request $request, Response $response, $args, $internal = 0)
    {
        $inputs = array();
        $inputs = $request->getParsedBody();
        ...
        $sql0 = "select gender,packageLevel, packageId from users inner join packages 
        ...
        try {
            ...
            if ($userGender == 0 && $data[0]['packageId'] !=11) {if ($userGender == 0 && $packageLevel < 4 && ($mariageKind == '183' || $mariageKind == '185')) {
                $responseData['status'] = 'error';
    ...
    public function openChat(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $sql4 = " select id,message,type,messageTime,DATE_FORMAT(messageTime, '%Y-%m-%d') as messageDate,
                    DATE_FORMAT(messageTime,'%H:%i:%s') as messageTimeOnly ,if(senderId=$userId,0,1) as owner,readed,
                    parent,parentMessage,parentType,played,voiceTime
                    from chatMessages where  chatId=:id and message like '%$search%' order by id desc limit 0,100";
    ...
    public function confirmVapPayment(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        try {
            ...
            $services = new Services($this->container);
            $services->sendEmail('support@zefaaf.net', 'Payment', $message);
       ...
       public function requestMobile(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        //check mobile requests limit for the user
        $checkLimit = "select packageMobileRequestLimit from users where id=$userId";
        $sth = $this->db->prepare($checkLimit);
        $sth->execute();
        $limit = $sth->fetchAll();
        if ($limit[0]['packageMobileRequestLimit'] == 0){
            $responseData['status'] = 'error';
            $responseData['errorCode'] =  'reach out requests limit';
            return JsonResponse::withJson($response, json_encode($responseData), 200);
        }

        //check if ignore list
        $ignored = $this->checkIgnored($userId, $otherId);
        if ($ignored == 1) {
            $responseData['status'] = 'error';
            $responseData['errorCode'] =  'ignore list';
        ...
        //get other user package level
        $packageLevel = "select packageLevel from packages where id= (select packageId from users where id=$otherId)";
        $sth = $this->db->prepare($packageLevel);
        $sth->execute();
        $otherUserPackage = $sth->fetchAll();

        $sql = "REPLACE into lists (userId,otherId,listType) values($userId,$otherId,7)";


        if ($otherUserPackage && $otherUserPackage[0]['packageLevel'] == 0 || $otherUserPackage[0]['packageLevel'] == 6) {
            //store in lists table with listType = 8 "approved" due to account is free
            $sql1 = "REPLACE into lists (userId,otherId,listType) values($otherId,$userId,8)";
            //decrease user mobile limit by 1 and execute it
            $sql2 = "update users set packageMobileRequestLimit = packageMobileRequestLimit -1 where id = $userId";
            $sth = $this->db->prepare($sql2);
            $sth->execute();
            //return mobile of other user
            $sql3 = "select mobile,mobileCode from users where id=$otherId";
            $sth = $this->db->prepare($sql3);
            $sth->execute();
            $mobile = $sth->fetchAll();
        ...
        try {
            ...
            $responseData['rowsCount'] = 1;
            if ($mobile){
                //get request limit for user
                $sql4 = "select packageMobileRequestLimit from users where id=$userId";
                $sth = $this->db->prepare($sql4);
                $sth->execute();
                $limit = $sth->fetchAll();
                //store new request for accepting
                $sth = $this->db->prepare($sql1);
                $sth->execute();
                $responseData['data'] = [
                    'mobile' => $mobile[0]["mobileCode"] . $mobile[0]["mobile"],
                    'count_limit' => $limit[0]['packageMobileRequestLimit']
                ];
            }
        } catch (\PDOException $e) {
    ...
    public function replyRequestMobile(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        try {
            ...
            if ($statues == 4) { //Approved
                ...
                $args['notiType'] = 8;
                $this->AddNotification($request, $response, $args);
                $sql1 = "update users set packageMobileRequestLimit = packageMobileRequestLimit -1 where id = $otherId";
                $sth = $this->db->prepare($sql1);
                $sth->execute();
                ...
                $args['listType'] = '8';
                $this->removeFromFavorites($request, $response, $args, 1);
                $sql2 = "delete from lists where userId =$otherId  and otherId = $userId and listType =7";
                $sth = $this->db->prepare($sql2);
                $sth->execute();
      ...
      public function getMarriageDetails(Request $request, Response $response, $args, $internal = 0)
    {
        $responseData = array();
        $inputs = array();
        $inputs = $request->getParsedBody();
        ...
        $sql = "select id, userId, messageDateTime ,reasonId , title , message, readed,owner, image, adminImage,otherId,reply from messages where userId = $userId";
    ...
    public function updateMarriageRequest(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        try {
        // Fetch existing data
        $sql = "SELECT * FROM messages WHERE id = :messageId AND userId = :userId";
        $sth = $this->db->prepare($sql);
        $sth->bindParam(':messageId', $messageId);
        $sth->bindParam(':userId', $userId);
        $sth->execute();
        $existingData = $sth->fetch();

        if (!$existingData) {
            $responseData['status'] = 'error';
            $responseData['errorCode'] = 'message not found or unauthorized';
            return JsonResponse::withJson($response, json_encode($responseData), 404);
        }

        // Merge inputs with existing data
        $realName = $inputs['realName'] ?? $existingData['realName'];
        $whats = isset($inputs['whats']) ? trim($inputs['whats'], '+') : $existingData['whats'];
        $age = $inputs['age'] ?? '';
        $mariageStatues = $inputs['mariageStatues'] ?? '';
        $mariageKind = isset($inputs['mariageKind']) ? ($inputs['mariageStatues'] == '1' ? 5 : $inputs['mariageKind']) : '';
        $residentCountryId = $inputs['residentCountryId'] ?? $existingData['residentCountryId'];
        $aboutMe = $inputs['aboutMe'] ?? '';
        $aboutOther = $inputs['aboutOther'] ?? '';
        $thanksMessage = $inputs['thanksMessage'] ?? '';

        if ((is_null($realName) || $realName == '' || is_null($whats) || strlen($whats) < 8
                || is_null($age) || $age == '' || $mariageStatues == '' ||
                //is_null($residentCountryId) || $residentCountryId == '' ||
                $mariageKind == '' ||
                is_null($aboutMe) || $aboutMe == '' || is_null($aboutOther) || $aboutOther == '' ||
        ...
        $fullMessage = "إسمي: $realName\n\nعمري: $age\n\nحالتي الإجتماعية: " .
            $fixedData[array_search($mariageStatues, array_column($fixedData, 'id'))]['title'] . "\n\nنوع الزواج: " .
            $fixedData[array_search($mariageKind, array_column($fixedData, 'id'))]['title'] . "\n\nرقم الواتس: $whats\n\nبعض من مواصفاتي: $aboutMe\n\nمواصفات شريكة (شريك) حياتي: $aboutOther\n\nكلمة شكر لفريق العمل: $thanksMessage\n";
        ...
        $sth = $this->db->prepare($sql);
        $sth->bindParam(':userId', $userId);
        $sth->bindParam(':fullMessage', $fullMessage);
        $sth->bindParam(':whats', $whats);
        $sth->bindParam(':realName', $realName);
        $sth->bindParam(':residentCountryId', $residentCountryId);
        $sth->bindParam(':messageId', $messageId);
        $sth->execute();
    ...
    public function deleteMarriageRequest(Request $request, Response $response, $args, $internal = 0)
    {
        $messageId = $args['messageId'];
        $userId = $request->getAttribute("userId");
        $inputs = $request->getParsedBody();
    ...
    public function getNewsTapeData(Request $request, Response $response, $args, $internal = 0)
    {
        $responseData = array();
        $inputs = array();
        $inputs = $request->getParsedBody();
        ...
        public function getMobileRequests(Request $request, Response $response, $args, $internal = 0)
    {
         $responseData = array();
        ...
        $sql = "SELECT users.id, users.userName, users.age, users.gender, users.available, users.lastAccess, users.packageId,
                users.residentCountryId, countries1.nameAr as residentCountryName, users.nationalityCountryId, countries2.nameAr as 
                    nationalityCountryName, users.cityId, cities.nameAr as cityName,
                users.mariageStatues,  users.age, if(available=0,1,available) as available, residentCountryId , lists.id as request_id FROM users INNER JOIN lists  INNER JOIN countries AS countries1 INNER JOIN countries AS countries2
                    INNER JOIN cities  INNER JOIN packages 
                    ON users.id = lists.userId AND users.residentCountryId = countries1.id AND users.nationalityCountryId = countries2.id
                    AND users.cityId=cities.id AND users.packageId = packages.id WHERE lists.otherId = $userId AND lists.listType = 8 AND 
                    users.active=1
                    AND (susbendedTillDate < CURDATE() or ISNull(susbendedTillDate))";
