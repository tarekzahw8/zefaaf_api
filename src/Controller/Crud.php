...
final class Crud
{
    ...
    public function reActiveUsers(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        //     error_log('PDOException: Failed to update users: ' . $e->getMessage());
    
        //     // Prepare the error response
        //     $responseData['status'] = 'error';
        //     $responseData['message'] = 'Failed to update users due to a database error: ' . $e->getMessage();
        // } catch (\Exception $e) {
        //     // Log the error with a detailed message
        //     error_log('Exception: Failed to update users: ' . $e->getMessage());
    ...
    public function uploadFile(Request $request, Response $response, $args, $internal = 0)
    {
     ...
     public function updateNewsTape(Request $request, Response $response, $args)
    {
        $inputs = array();
        $inputs = $request->getParsedBody();
        $responseData = array();
        ...
        $newsData = $fields->newsTape;
        $sql = "update news set data = $newsData where id = 1";
    ...
    public function getHome(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $sql4 = " select count(id) as newMessages from messages where owner=0 and readed=0 and reasonId = 4 ";
    ...
    public function getPurchases(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        public function listMarriageRequests(Request $request, Response $response, $args, $internal = 0)
    {
        $responseData = array();
        $pageSize = $this->pageSize;
        ...
        $sql = "SELECT 
            messages.*, 
            users1.userName, 
            IF(messages.otherId = 0, '', users2.userName) AS complaintUser, 
            CASE 
                WHEN messages.userId = 0 AND messages.residentCountryId = 0 THEN 'الكل'
                WHEN messages.userId = 0 THEN countries.nameAr 
                ELSE countries.nameAr 
            END AS residentCountryName,
            CASE 
                WHEN messages.userId = 0 THEN 'الكل'
                ELSE CASE users1.gender 
                    WHEN 0 THEN 'ذكر'
                    WHEN 1 THEN 'انثى'
                    ELSE 'الكل'
                END 
        ...
        FROM 
            messages 
        LEFT JOIN 
            users AS users1 ON messages.userId = users1.id 
        LEFT JOIN 
            users AS users2 ON messages.otherId = users2.id 
        LEFT JOIN 
            countries ON 
                CASE 
                    WHEN messages.userId = 0 THEN messages.residentCountryId 
                    ELSE users1.residentCountryId 
                END = countries.id
        WHERE 
            messages.active = 1
        AND 
...
$sql = "SELECT 
        ...
        LEFT JOIN 
            countries ON 
                CASE 
                    ...
                    ELSE users1.residentCountryId 
                END = countries.id
        WHERE messages.reasonId IN (0,1,2,3)
        AND messages.active = 1";


        $sql .= ($search != ' ') ? " and (users1.userName like '%$search%' or users2.userName like '%$search%' or messages.title like '%$search%' or messages.message like '%$search%')" : "";
        
        $sql .= ($reasonId >= 0) ? " and messages.reasonId=$reasonId" : " ";
    ...
    public function listUsers(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        users.residentCountryId,users.workField,fixedData.title as workFieldTitle,countries1.nameAr as residentCountryName,
        users.nationalityCountryId,countries2.nameAr as nationalityCountryName,users.cityId,cities.nameAr as cityName,users.creationDate,
        users.lastAccess,
        users.mariageStatues,users.active,users.susbendedTillDate,if(users.susbendedTillDate >= CURDATE(), 1, 0) as susbended,
        users.mobileType,users.detectedCountry from users 
            inner join countries as countries1 on users.residentCountryId = countries1.id
            inner join countries as countries2 on users.nationalityCountryId = countries2.id
            inner join cities on users.cityId = cities.id
            inner join fixedData on users.workField = fixedData.id
        where  1 = 1";
        ...
        $sql .= isset($userGender) ? " and users.gender=$userGender" : "";
    ...
    public function adminNotification(Request $request, Response $response, $args)
    {
        ...
        $countries = isset($inputs['country']) ? $inputs['country'] : [] ;
        

        if ($gender == 'all' && in_array('all',$countries)){
        ...
        try {
            ...
            if (isset($userId)) {
                ...
                $residentCountryIds = in_array('all',$countries) ? ['0'] : $countries;
                foreach ($residentCountryIds as $residentCountryId){
    ...
    public function subscripePackage(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $sql0 = "select id,usdValue,mobileRequestLimit , packageLevel from packages where id=$packageId";
        $sth = $this->db->prepare($sql0);
        $sth->execute();
        $data = $sth->fetchAll();
        $paymentValue =  $data[0]['usdValue'];
        $mobileLimit =  $data[0]['mobileRequestLimit'];
        $packageLevel =  $data[0]['packageLevel'];
        $canEdit = 0; //user will has permission to edit profile
        if($packageLevel == 3 || $packageLevel == 2 || $packageLevel == 1 || $packageLevel == 0){ 
            //in this cases user will not edit profile
            $canEdit = 1;
        }
        ...
        $sql = "insert into purchases (userId,packageId,paymentRefrence,paymentValue,agentId,transactionId) 
          ...
          if ($packageId == 11 || $packageId == '11') {
          dd(1);
             $packageDays = '10 YEAR';
             $sql1 = "UPDATE users 
             SET packageRenewDate = DATE_ADD(NOW(), INTERVAL $packageDays),
                 packageId = $packageId
             WHERE users.id = $userId";
        }
        else {
             $sql1 = "UPDATE users 
             SET packageRenewDate = DATE_ADD(IF(users.packageId = 0 OR users.packageId = 11, CURDATE(), packageRenewDate),
                              INTERVAL (SELECT validFor FROM packages WHERE id = $packageId) DAY),
                 packageId = $packageId,
                 packageMobileRequestLimit = $mobileLimit,
                 canUpdateProfile = $canEdit
             WHERE users.id = $userId";
        }
...
public function readAllMessages(Request $request, Response $response, $args, $internal = 0)
{
    ...
    public function listPendingPhotos(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        //$sql = "select id,userName,gender, tempProfileImage,photoUploadeDate,residentCountryId  from users where active = 1 and !isNull(tempProfileImage)";
    $sql = "select 
            u.id, u.userName, u.gender, u.tempProfileImage, u.photoUploadeDate, u.residentCountryId, 
            c.nameAr as residentCountryName 
            from 
            users u
            inner join
            countries c on u.residentCountryId = c.id
            where 
            u.active = 1 and !isNull(tempProfileImage)";
    ...
    public function sendGeneralMessage(Request $request, Response $response, $args, $internal = 0)
    {
        ...
        $title = $inputs['title'];
        $message = $inputs['message'];
        $multiImgs = json_encode($inputs['adminImage']);
        $adminImage = '';
        ...
        $gender = $inputs['gender'];
        $reasonId = isset($inputs['reasonId']) ? $inputs['reasonId'] : 4;
        ...
        // $residentCountryId = ($country == 'all') ? '0' :  intval($country);
         ...
         if (in_array('all', $countries)) {
        $residentCountryIds = [0]; 
         } else {
        $residentCountryIds = array_map('intval', $countries);  
        }
        ...
        try {
            ...
            $sql = "insert into messages (userId,reasonId,title,message,owner,adminImage,mesGender,residentCountryId,multi_image) 
                    values 
                    ('0','$reasonId','$title','$message','1',null,'$mesGender','$residentCountryId','$multiImgs')";
            // echo $sql;
